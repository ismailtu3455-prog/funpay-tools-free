...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request && request.action === 'fetchDonaters') {
        (async () => {
            ...
            const customWorkerUrl = 'https://funpayfreetoolsbot.ismailtu3455.workers.dev/donaters.json';
            ...
            for (const url of sources) {
                try {
                    ...
                    if (json && typeof json === 'object' && Object.keys(json).length > 0) {
                        Object.assign(combinedData, json);
                        fetchedAny = true;
                        break;
                    }
                } catch (e) {}
            ...
            try {
                const res = await fetch(`${customWorkerUrl}?t=${bust}`, fetchOpts);
                if (res.ok) {
                    const customJson = await res.json();
                    if (customJson && typeof customJson === 'object') {
                        Object.assign(combinedData, customJson);
                        fetchedAny = true;
                    }
                }
            } catch (e) {}
            ...
            sendResponse({ success: false, error: 'Failed to fetch from all sources' });
        })();
        return true;
    }
