    ...
    try {
        ...
        if (gate) {
            gate.querySelector('.fpt-gc-gate-inner').innerHTML =
                ...
                '<div class="fpt-gc-gate-sub">Нажми кнопку и откроется наш бот и автоматически верифицирует вас. Вы можете скопировать код ниже и написать в @funpayfreetoolsbot, если не можете открыть ссылку.</div>' +
                `<a href="${deeplink}" target="_blank" class="btn" id="fpt-gc-open-bot">Открыть бота и подтвердить</a>` +
                `<div class="fpt-gc-code">Если у вас нет Telegram на компьютере, введите код с телефона в бота @funpayfreetoolsbot: <b>${_fptGcEscape(code)}</b></div>` +
