...
(function() {
    ...
    function injectGlobalCSS() {
        ...
        for (const [nick, key] of Object.entries(donatersMap)) {
            ...
            try {
                ...
                const sclFix = (anims.includes('wave') && parseInt(cfg.scl) <= 100) ? '300' : cfg.scl;

                if (anims.includes('glow')) {
                    cssRules += `@keyframes fpt-glow-${clsName} { 0% { filter: drop-shadow(0 0 8px ${shadow}); } 100% { filter: drop-shadow(0 0 20px ${shadow}) brightness(1.2); } }\n`;
                }
                if (anims.includes('pulse')) {
                    cssRules += `@keyframes fpt-pulse-${clsName} { 0%, 100% { opacity: 1; filter: drop-shadow(0 0 18px ${shadow}) brightness(1.25); } 50% { opacity: 0.6; filter: drop-shadow(0 0 4px rgba(0,0,0,0.1)) brightness(0.9); } }\n`;
                }
    ...
    function renderLoop(now) {
        ...
        canvases.forEach(item => {
            ...
            if(cfg.ov === 'fire') {
                ...
                for(let i=0; i<3; i++) {
                    if(Math.random() < 0.7) parts.push({ x: textStartX + 4 + Math.random() * (textWidth - 8), y: textTopY + Math.random() * 5 + 2, s: Math.random() * 5 + 2, sy: Math.random() * -1.5 - 0.5, sx: (Math.random() - 0.5) * 0.8, a: 1 });
                }
                for(let i=parts.length-1; i>=0; i--) {
                    let p = parts[i]; p.y += p.sy; p.x += p.sx + Math.sin(p.y * 0.1) * 0.4; p.s *= 0.93; p.a -= 0.03;
                    ctx.fillStyle = getPartColor(cfg, 255, Math.max(0, 230 * p.a), 0, p.a * 1.2);
                    ctx.beginPath(); ctx.arc(p.x, p.y, p.s, 0, Math.PI*2); ctx.fill();
                    if(p.a <= 0 || p.s <= 0.3) parts.splice(i,1);
                }
            } else if(cfg.ov === 'snow') {
                ...
                ctx.globalCompositeOperation = 'screen';
                if(Math.random()<0.15) {
                    let startX = textStartX - 10 + Math.random()*(textWidth+20);
                    parts.push({
                        x: startX, 
                        y: textBaseY + 15, 
                        s: Math.random()*4 + 2, 
                ...
                for(let i=parts.length-1; i>=0; i--) { 
                    let p = parts[i]; 
                    p.ang += 0.03; 
                    p.x = p.ox + Math.sin(p.ang)*12; 
                    p.y += p.sy; 
                    p.a -= 0.01; 
                    
                    // Main bubble body (semi-transparent)
                    ctx.beginPath(); 
                    ctx.arc(p.x, p.y, p.s, 0, Math.PI*2); 
                    ctx.fillStyle = getPartColor(cfg, 255, 255, 255, p.a * 0.15); 
                    ctx.fill(); 
                    
                    // Bubble outline
                    ctx.lineWidth = 1;
                    ctx.strokeStyle = getPartColor(cfg, 255, 255, 255, p.a * 0.7);
                    ctx.stroke();

                    // Bubble light reflection (highlight)
                    ctx.beginPath();
                    ctx.arc(p.x - p.s*0.3, p.y - p.s*0.3, p.s*0.2, 0, Math.PI*2);
                    ctx.fillStyle = getPartColor(cfg, 255, 255, 255, p.a * 0.9);
                    ctx.fill();
    ...
    window.renderEpicPreviews = function() {
        ...
        const presets = [
            "FPT-STYLE-eyJjMSI6IiNmZjg4MDAiLCJjMiI6IiNmZjAwMDAiLCJjMyI6IiNmZmZmMDAiLCJhbmciOiI5MCIsInNjbCI6IjEwMCIsInNwZCI6IjIiLCJhbiI6WyJnbG93Iiwid2F2ZSJdLCJvdiI6ImZpcmUiLCJwYyI6IiNmZjU1MDAifQ==",
            "FPT-STYLE-eyJjMSI6IiMwMGZmY2MiLCJjMiI6IiMwMDY2ZmYiLCJjMyI6bnVsbCwiYW5nIjoiNDUiLCJzY2wiOiIxMDAiLCJzcGQiOiIzIiwiYW4iOlsiZ2xpdGNoIl0sIm92IjoibWF0cml4IiwicGMiOiIjMDBmZjAwIn0=",
            "FPT-STYLE-eyJjMSI6IiNmZmQ3MDAiLCJjMiI6IiNmZmZmZmYiLCJjMyI6IiNmZmFhMDAiLCJhbmciOiI5MCIsInNjbCI6IjEwMCIsInNwZCI6IjQiLCJhbiI6WyJnbG93Il0sIm92Ijoic3BhcmtzIiwicGMiOiIjZmZmZjU1In0=",
            "FPT-STYLE-eyJjMSI6IiNmZjAwZmYiLCJjMiI6IiMwMGZmZmYiLCJjMyI6bnVsbCwiYW5nIjoiOTAiLCJzY2wiOiIxMDAiLCJzcGQiOiI1IiwiYW4iOlsicHVsc2UiXSwib3YiOiJvcmJzIiwicGMiOiIjZmZmZmZmIn0=",
            "FPT-STYLE-eyJjMSI6IiNmZjZmZmYiLCJjMiI6IiMwMGFhZmYiLCJjMyI6bnVsbCwiYW5nIjoiMTgwIiwic2NsIjoiMTAwIiwic3BkIjoiMyIsImFuIjpbImdsb3ciXSwib3YiOiJzbm93IiwicGMiOiIjZmZmZmZmIn0=",
            "FPT-STYLE-eyJjMSI6IiNjY2NjZmYiLCJjMiI6IiM0NDQ0ZmYiLCJjMyI6bnVsbCwiYW5nIjoiOTAiLCJzY2wiOiIxMDAiLCJzcGQiOiIxIiwiYW4iOlsiZ2xpdGNoIl0sIm92IjoibGlnaHRuaW5nIiwicGMiOiIjZmZmZmZmIn0=",
            "FPT-STYLE-eyJjMSI6IiM1NTAwYWEiLCJjMiI6IiNmZjAwZmYiLCJjMyI6bnVsbCwiYW5nIjoiNDUiLCJzY2wiOiIxMDAiLCJzcGQiOiI4IiwiYW4iOlsid2F2ZSJdLCJvdiI6InN0YXJzIiwicGMiOiIjZmZmZmZmIn0=",
            "FPT-STYLE-eyJjMSI6IiM4ODg4ODgiLCJjMiI6IiNjY2NjY2MiLCJjMyI6bnVsbCwiYW5nIjoiMCIsInNjbCI6IjEwMCIsInNwZCI6IjYiLCJhbiI6WyJwdWxzZSJdLCJvdiI6InNtb2tlIiwicGMiOiIjOTk5OWFhIn0=",
            "FPT-STYLE-eyJjMSI6IiMwMGZmMDAiLCJjMiI6IiMwMDU1MDAiLCJjMyI6bnVsbCwiYW5nIjoiOTAiLCJzY2wiOiIxMDAiLCJzcGQiOiIyIiwiYW4iOlsiZ2xvdyJdLCJvdiI6Im1hdHJpeCIsInBjIjoiIzAwZmYwMCJ9",
            "FPT-STYLE-eyJjMSI6IiNmZjAwMDAiLCJjMiI6IiMwMGZmMDAiLCJjMyI6IiMwMDAwZmYiLCJhbmciOiI0NSIsInNjbCI6IjEwMCIsInNwZCI6IjQiLCJhbiI6WyJ3YXZlIiwiZ2xvdyJdLCJvdiI6Im9yYnMiLCJwYyI6IiNmZmZmZmYifQ=="
        ];

        const presetNames = [
            "🔥 Огонь", "🤖 Кибер", "👑 Золото", "🟣 Неон", "❄️ Лёд", 
            "⚡ Молния", "🌌 Космос", "💨 Дым", "💻 Хакер", "🌈 Радуга"
        ];
        ...
        presets.forEach((preset, i) => {
            ...
            row.style.cssText = "display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #1e2030; padding: 14px 12px; position: relative; z-index: 1; border-radius: 6px; overflow: hidden; transition: background 0.25s, z-index 0s;";
            ...
            nameCol.style.cssText = "font-size: 16px; position: relative; min-width: 0; flex: 1;";
            
            const textNode = document.createTextNode(nick);
            nameCol.appendChild(textNode);
            
            const infoCol = document.createElement('div');
            infoCol.style.cssText = "font-size: 12px; color: #5a5f7a; display: flex; align-items: center; gap: 10px; flex-shrink: 0;";
            
            const titleSpan = document.createElement('span');
            titleSpan.textContent = presetNames[i] || `Стиль ${i+1}`;
            ...
            selectBtn.onclick = () => {
                window.open(`https://t.me/funpayfreetoolsbot?start=style_${i+1}`, '_blank');
            };
            
            infoCol.appendChild(titleSpan);
            infoCol.appendChild(selectBtn);
